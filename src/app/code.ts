export class Code {
    ...
    toggleFound(): void {
        ...
        this.timestampResolved = this.resolved ? (new Date).getTime() : null;
    }

    getTimeMissing(): string {
        const start = this.timestampResolved ? this.timestampResolved : (new Date).getTime();
        return Code.getTimeString(start - this.timestampLogged);
    }

    // TODO:: extract sev constants
    getTimeMissingCss(withFound: boolean): string {
        if(withFound && this.resolved) {
            return 'found';
        }
        ...
        const start = this.timestampResolved ? this.timestampResolved : (new Date).getTime();
        const missing = start - this.timestampLogged;
